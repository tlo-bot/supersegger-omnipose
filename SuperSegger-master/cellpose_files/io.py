...
# Now saves to directories     
def save_masks(images, masks, flows, file_names, png=True, tif=False, channels=[0,0], skel=True,save_extras=False,dir_above=False):
    ...
    if isinstance(masks, list):
        for image, mask, flow, file_name in zip(images, masks, flows, file_names):
            save_masks(image, mask, flow, file_name, png=png, tif=tif, skel=skel, save_extras=save_extras,dir_above=dir_above)
        return
...
#     base = os.path.splitext(file_names)[0]
    if dir_above:
        basedir = Path(file_names).parent.parent.absolute() #go up a level to save in its own folder
    else:
        basedir = Path(file_names).parent.absolute()
    
    basename = os.path.splitext(os.path.basename(file_names))[0]
    maskdir = os.path.join(basedir,'masks')
    outlinedir = os.path.join(basedir,'outlines')
    ncolordir = os.path.join(basedir,'ncolor_masks')
    flowdir = os.path.join(basedir,'flows')
    ...
    # demark the original cellpose files with _OG 
    if skel:
        suffix = ''
    else:
        suffix = '_OG'
...
#             print(os.path.join(maskdir,basename + '_cp_masks' + suffix + ext))
    ...
    if png and MATPLOTLIB and not min(images.shape) > 3:
        ...
        fig.savefig(os.path.join(basedir,basename + '_cp_output' + suffix + '.png'), dpi=300)
        plt.close(fig)

    if masks.ndim < 3 and save_extras: 
...
#         outlines_to_text(base, outlines) #no need for these right now
...
#         imsave(os.path.join(ncolordir, basename + '_cp_ncolor_masks' + suffix + '.png'),  ncolorlabel(masks))
        imsave(os.path.join(flowdir, basename + '_flows' + suffix + '.png'), (flows[0]*65535).astype(np.uint16))
