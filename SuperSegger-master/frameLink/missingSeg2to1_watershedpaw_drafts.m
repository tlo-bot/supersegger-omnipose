...
if numel(regR)==2 && numel(regF)==2 %if 2:1:2, also use f info
    ...
    regisrf_mask = regisr_mask + regisf_mask; %adding 0.5,1 and 2,4 should have overlap 2.5, 5
    
    r1 = (regisrf_mask==2.5); %one overlap labeled by 2.5 
    r2 = (regisrf_mask==5); %other overlap labeled by 5
    sr1 = bwdist(~r1);
    sr2 = bwdist(~r2);
    sr12 = -(sr1 + sr2); %consider gaussian blur after this line?
...
%assumes forward frame has 2 regions (assumes mask correct, r:c:f = 2:1:2)
...
%% if area change causes error between current and forward frame, average watershed between reverse and forward frames
%don't use forward frame as default in case forward frame is also not
%segmented properly (ie 2:1:1)
